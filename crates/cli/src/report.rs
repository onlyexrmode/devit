...
pub fn read_junit<P: AsRef<Path>>(p: P, flaky_list: Option<&[String]>) -> Result<(u32, u32, Option<u32>)> {
    let s = fs::read_to_string(&p).with_context(|| format!("read junit at {}", p.as_ref().display()))?;
    // naive: look for attributes on testsuite
    let mut total = 0u32; let mut failed = 0u32; let mut flaky_failed = 0u32;
    for line in s.lines() {
    ...
    if total == 0 {
        ...
        for line in s.lines() {
            if line.contains("<testcase") { total += 1; }
            if line.contains("<failure") { failed += 1; }
        }
    }
    ...
    Ok((total, failed, if flaky_list.is_some() { Some(flaky_failed) } else { None }))
}
...
pub fn read_sarif<P: AsRef<Path>>(p: P) -> Result<(u32, u32, u32)> {
    ...
    let mut errors = 0u32; let mut warnings = 0u32; let mut rules = 0u32;
    let runs = v.get("runs").and_then(|x| x.as_array()).cloned().unwrap_or_default();
    for run in runs.iter() {
        ...
        if let Some(results) = run.get("results").and_then(|r| r.as_array()) {
            for res in results {
                ...
                match lvl { "error" => errors += 1, "warning" => warnings += 1, _ => {} }
            }
        }
    }
...
pub fn summarize(junit_path: &Path, sarif_path: &Path, cfg: &QualityCfg, flaky: Option<&[String]>) -> Result<QualitySummary> {
    ...
    match read_junit(junit_path, flaky) {
        Ok((t, f, ff)) => {
            ...
            if let Some(x) = ff { 
                ...
                if x > 0 { sum.notes.push(format!("{} flaky failures ignored", x)); }
            } else { sum.tests_failed = f; }
        },
        Err(e) => {
            if cfg.fail_on_missing_reports { return Err(e); }
            sum.notes.push(format!("junit missing: {}", e));
        }
    }
    match read_sarif(sarif_path) {
        Ok((e, w, r)) => { sum.lint_errors = e; sum.lint_warnings = w; sum.sarif_rules = r; },
        Err(e) => {
            if cfg.fail_on_missing_reports { return Err(e); }
...
pub fn check_thresholds(sum: &QualitySummary, cfg: &QualityCfg) -> bool {
    if sum.tests_failed > cfg.max_test_failures { return false; }
    if sum.lint_errors > cfg.max_lint_errors { return false; }
    if !cfg.allow_lint_warnings && sum.lint_warnings > 0 { return false; }
    true
}
...
pub fn summary_markdown(junit: &Path, sarif: &Path, out: &Path) -> Result<()> {
    ...
    if let Some(dir) = out.parent() { let _ = std::fs::create_dir_all(dir); }
    std::fs::write(out, md)?;
    Ok(())
}
